...
/** 
 * Modified heavily for mos (from linux 2.2.16)
 * $Id: binfmt_elf.c,v 1.2 2014/06/03 20:24:36 mvines Exp $
 */

/* uncomment this define if you want the ELF loader to be noisy */
//#define __VERBOSE_SYSCALL__


#include <ps/binfmt/elf.h>
#include <ps/binfmt/a.out.h>
                                     ...
                                     unsigned long *interp_load_addr)
{
  ...
  for (i=0; i<interp_elf_ex->e_phnum; i++, eppnt++) {
    if (eppnt->p_type == PT_LOAD) {
      int elf_type = 0; // | MAP_DENYWRITE;
      int elf_prot = 0;
      unsigned long vaddr = 0;
      unsigned long k, map_addr;

      if (eppnt->p_flags & PF_R) elf_prot =  0;
      if (eppnt->p_flags & PF_W) elf_prot |= 0;
      if (eppnt->p_flags & PF_X) elf_prot |= 0;
      vaddr = eppnt->p_vaddr;
  ...
  if (last_bss > elf_bss)
    do_mmap(-1, elf_bss, last_bss - elf_bss, 0,
            0, 0);
...
//  current->mm->rss = 0;
  ...
  for(i = 0, elf_ppnt = elf_phdata; i < elf_ex.e_phnum; i++, elf_ppnt++) {
    ...
    if (elf_ppnt->p_flags & PF_R) elf_prot |= 0;
    if (elf_ppnt->p_flags & PF_W) elf_prot |= 0;
    if (elf_ppnt->p_flags & PF_X) elf_prot |= 0;

    elf_flags = 0; // |MAP_DENYWRITE|MAP_EXECUTABLE;
