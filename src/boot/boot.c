...
_START static void setup_gdt() {
    ...
    _gdt[0] = 0;
    _gdt[KERNEL_CODE_SELECTOR / 8] = MAKE_SEG_DESC(0, ADDRESS_LIMIT, SEG_CLASS_DATA, 10, KERNEL_PRIVILEGE, SEG_BASE_4K);
    _gdt[KERNEL_DATA_SELECTOR / 8] = MAKE_SEG_DESC(0, ADDRESS_LIMIT, SEG_CLASS_DATA, 2, KERNEL_PRIVILEGE, SEG_BASE_4K);
    _gdt[USER_CODE_SELECTOR / 8] = MAKE_SEG_DESC(0, ADDRESS_LIMIT, SEG_CLASS_DATA, 8, USER_PRIVILEGE, SEG_BASE_4K);
    _gdt[USER_DATA_SELECTOR / 8] = MAKE_SEG_DESC(0, ADDRESS_LIMIT, SEG_CLASS_DATA, 2, USER_PRIVILEGE, SEG_BASE_4K);
    _gdt[TSS_SELECTOR / 8] = MAKE_SEG_DESC(0, 0x67, SEG_CLASS_SYSTEM, 9, KERNEL_PRIVILEGE, SEG_BASE_1);  // tmp
...
_START static void mm_get_phy_mem_bound(multiboot_info_t* mb) {
    ...
    unsigned long long* _phy_mem_low = GET_BOOT_ADDR(unsigned long long*, &phy_mem_low);
    unsigned long long* _phy_mem_high = GET_BOOT_ADDR(unsigned long long*, &phy_mem_high);
    memory_map_t* map;
...
_START void boot_stage1(multiboot_info_t* mb, unsigned int magic) {
    ...
    *(multiboot_info_t**)((unsigned)dst - KERNEL_OFFSET) = (multiboot_info_t*)((unsigned)src + KERNEL_OFFSET);
