...
static void do_cpuid(unsigned leaf, unsigned *eax, unsigned *ebx,
...
static void fill(void *buf, size_t size)
{
	...
	for (i = 0; i < ncpu; i++) {
		...
		if (edx & (1u <<  0)) { sprintf(p, " fpu");     p += strlen(p); }
		if (edx & (1u <<  1)) { sprintf(p, " vme");     p += strlen(p); }
		if (edx & (1u <<  4)) { sprintf(p, " tsc");     p += strlen(p); }
		if (edx & (1u <<  5)) { sprintf(p, " msr");     p += strlen(p); }
		if (edx & (1u <<  8)) { sprintf(p, " cx8");     p += strlen(p); }
		if (edx & (1u <<  9)) { sprintf(p, " apic");    p += strlen(p); }
		if (edx & (1u << 11)) { sprintf(p, " sep");     p += strlen(p); }
		if (edx & (1u << 15)) { sprintf(p, " cmov");    p += strlen(p); }
		if (edx & (1u << 19)) { sprintf(p, " clflush"); p += strlen(p); }
		if (edx & (1u << 23)) { sprintf(p, " mmx");     p += strlen(p); }
		if (edx & (1u << 24)) { sprintf(p, " fxsr");    p += strlen(p); }
		if (edx & (1u << 25)) { sprintf(p, " sse");     p += strlen(p); }
		if (edx & (1u << 26)) { sprintf(p, " sse2");    p += strlen(p); }
		if (edx & (1u << 28)) { sprintf(p, " ht");      p += strlen(p); }

		/* Feature flags from CPUID leaf 1 ECX */
		if (ecx & (1u <<  0)) { sprintf(p, " sse3");    p += strlen(p); }
		if (ecx & (1u <<  9)) { sprintf(p, " ssse3");   p += strlen(p); }
		if (ecx & (1u << 19)) { sprintf(p, " sse4_1");  p += strlen(p); }
		if (ecx & (1u << 20)) { sprintf(p, " sse4_2");  p += strlen(p); }
		if (ecx & (1u << 23)) { sprintf(p, " popcnt");  p += strlen(p); }
		if (ecx & (1u << 30)) { sprintf(p, " rdrand");  p += strlen(p); }
