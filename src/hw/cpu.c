...
cpu_struct cpus[MAX_CPUS] = { [0] = { .sched_level = 1 } };
volatile int ncpus = 1; /* BSP is CPU 0, counted at boot */
static volatile int cpu_topology_ready;

unsigned long long *cpu_gdt(void)
{
	extern unsigned long long gdt[];
	if (!cpu_topology_ready)
		return gdt;
	return cpu_current()->gdt;
}

static void cpu_load_gdt(cpu_struct *cpu)
{
	extern unsigned long long gdt[];
	extern unsigned short gdt_size;
	unsigned long long gdtr;
	memcpy(cpu->gdt, gdt, sizeof(cpu->gdt));
	gdtr = MAKE_GDTR_OPERAND(gdt_size - 1, cpu->gdt);
	SET_GDT(gdtr);
}
...
{
	if (ncpus > 1)
		apic_broadcast_ipi(IPI_VECTOR_SCHED);
}
...
static void cpu_load_tss(int cpu_id, tss_struct *tss)
{
	...
	cpus[cpu_id].gdt[sel / 8] = MAKE_SEG_DESC(base, TSS_SEG_LIMIT, SEG_CLASS_SYSTEM, 9,
