...
{
	if (!vga_enabled || (_resolution_x != _hw_resolution_x) ||
	    (_resolution_y != _hw_resolution_y)) {
		char *dst = vidptr;
		...
		unsigned next_row = _fb_buffer + VGA_RESOLUTION_X *
							 (VGA_COLOR_DEPTH / 8) *
							 _fb_font_height;
		...
		unsigned row_pixel_size = VGA_RESOLUTION_X * _fb_font_height *
					  (VGA_COLOR_DEPTH / 8);

		char *next_char_row = _fb_text + _window_char_width;
		unsigned txt_copy_size =
			_window_char_width * (_window_char_height - 1);
		char *last_char_row =
			_fb_text +
			_window_char_width * (_window_char_height - 1);
		...
		// first clear cursor
		{
			...
			if (val == ' ' || val == '\0' || val == '\n' ||
			    val == '\r' || val == '\t' || val == '\b') {
				fb_write_char(CUR_COL, CUR_ROW, 130,
					      VGA_COLOR_BLACK);
			}
		}
...
void tty_clear()
{
	...
	if (!vga_enabled) {
		...
		src = _fb_buffer;
		len = VGA_RESOLUTION_X * VGA_RESOLUTION_Y *
		      (VGA_COLOR_DEPTH / 8);
		memset(src, 0, len);
	}
}
