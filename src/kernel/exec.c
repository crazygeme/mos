...
static void ps_get_argc_envc(const char* file, char** argv, char** envp, unsigned* argv_len, unsigned* envp_len) {
    char* tmp = 0;
    int i = 0;
...
static unsigned ps_setup_v(char* file, int argc, char** argv, int envc, char** envp, unsigned top, mos_binfmt* exec) {
    int i = 0;
    char* esp = (char*)(top);
...
int sys_execve(const char* file, char** argv, char** envp) {
     ...
     */
    if (firstline[0] == 0x7f && firstline[1] == 'E' && firstline[2] == 'L' && firstline[1] != 'F') {
        vm_free(firstline, 1);
        firstline = NULL;
