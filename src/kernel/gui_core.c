...
static int load_bmp(const char* filename) {
    ...
    for (y = 0; y < height; ++y) {
        for (x = 0; x < width; ++x) {
            ...
            /* Extract the color */
            if (bpp == 24) {
                color = (bufferb[i + 3 * x] & 0xFF) + (bufferb[i + 1 + 3 * x] & 0xFF) * 0x100 +
                        (bufferb[i + 2 + 3 * x] & 0xFF) * 0x10000;
            } else if (bpp == 32) {
                color = (bufferb[i + 4 * x] & 0xFF) * 0x1000000 + (bufferb[i + 1 + 4 * x] & 0xFF) * 0x100 +
                        (bufferb[i + 2 + 4 * x] & 0xFF) * 0x10000 + (bufferb[i + 3 + 4 * x] & 0xFF) * 0x1;
            }
            /* Set our point */
