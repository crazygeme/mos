...
static unsigned gui_gradually_color(unsigned from, unsigned to, int height,
				    int current)
{
	unsigned ra, ga, ba, rb, gb, bb;
	unsigned rs, gs, bs;
				 ...
				 unsigned from, unsigned to)
{
	...
	for (j = y; j < (y + height); j++) {
		for (i = x; i < (x + width); i++) {
			unsigned c =
				gui_gradually_color(from, to, height, (j - y));
			fb_set_point(i, j, c);
		}
	}
}
		...
		for (x = 0; x < width; ++x) {
			...
			if (bpp == 24) {
				color = (bufferb[i + 3 * x] & 0xFF) +
					(bufferb[i + 1 + 3 * x] & 0xFF) *
						0x100 +
					(bufferb[i + 2 + 3 * x] & 0xFF) *
						0x10000;
			} else if (bpp == 32) {
				color = (bufferb[i + 4 * x] & 0xFF) *
						0x1000000 +
					(bufferb[i + 1 + 4 * x] & 0xFF) *
						0x100 +
					(bufferb[i + 2 + 4 * x] & 0xFF) *
						0x10000 +
					(bufferb[i + 3 + 4 * x] & 0xFF) * 0x1;
			}
			/* Set our point */
			fb_set_point(_hw_resolution_x - (off_x + x) - 1,
				     _hw_resolution_y - (off_y + y) - 1, color);
		}
		i += row_width;
	}
	ret = 1;
