...
_START static unsigned long long make_gate(void (*function)(void), int dpl, int type) {
    unsigned int e0, e1;
...
_START static unsigned long long make_intr_gate(void (*function)(void), int dpl) {
...
_START static unsigned long long make_trap_gate(void (*function)(void), int dpl) {
...
_START static unsigned long long make_idtr_operand(unsigned short limit, void* base) {
...
typedef unsigned long long (*fpmake_seg_desc)(unsigned int base, unsigned int limit, unsigned int class, int type,
                                              int dpl, unsigned int granularity);

_START static unsigned long long make_seg_desc(unsigned int base, unsigned int limit, unsigned int class, int type,
                                               int dpl, unsigned int granularity) {
    unsigned e0, e1;
...
_START static unsigned long long make_gdtr_operand(unsigned short limit, void* base) {
    return limit | ((unsigned long long)(unsigned int)base << 16);
}

#define RELOAD_TSS(SELECTOR) __asm__("ltr %w0" : : "q"(SELECTOR));

#define RELOAD_GDT(gdtr) __asm__("lgdt %0" : : "m"(gdtr));
...
static void virtual_setup_gdt() {
    ...
    virtual_gdt[KERNEL_CODE_SELECTOR / 8] =
            make_seg_desc(0, ADDRESS_LIMIT, SEG_CLASS_DATA, 10, KERNEL_PRIVILEGE, SEG_BASE_4K);
    virtual_gdt[KERNEL_DATA_SELECTOR / 8] =
            make_seg_desc(0, ADDRESS_LIMIT, SEG_CLASS_DATA, 2, KERNEL_PRIVILEGE, SEG_BASE_4K);
    virtual_gdt[USER_CODE_SELECTOR / 8] =
            make_seg_desc(0, ADDRESS_LIMIT, SEG_CLASS_DATA, 8, USER_PRIVILEGE, SEG_BASE_4K);
    virtual_gdt[USER_DATA_SELECTOR / 8] =
            make_seg_desc(0, ADDRESS_LIMIT, SEG_CLASS_DATA, 2, USER_PRIVILEGE, SEG_BASE_4K);
