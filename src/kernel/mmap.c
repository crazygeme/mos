...
void vm_add_map(vm_struct_t vm, unsigned begin, unsigned end, void* fd, int offset) {
    ...
    if (!pair) {
        ...
        if (key->end < conflict_key->end && key->begin <= conflict_key->begin && key->end > conflict_key->begin) {
            // case 3
            // -------| region1 | ----------
            // -----------| region2 | ------
...
int do_mmap_kernel(unsigned int _addr, unsigned int _len, unsigned int prot, unsigned int flags, void* inode,
                   unsigned int offset) {
...
int do_mmap(unsigned int _addr, unsigned int _len, unsigned int prot, unsigned int flags, int fd, unsigned int offset) {
    void* node;
    task_struct* cur = CURRENT_TASK();
    // FIXME
