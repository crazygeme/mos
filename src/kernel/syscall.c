...
static int sys_llseek(int fd, unsigned offset_high, unsigned offset_low, uint64_t* result, unsigned int whence);
static int sys_select(int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, const struct timespec* timeout);
static int sys_newselect(int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, const struct timespec* timeout,
                         void* sigmask);
...
static int sys_mmap(struct mmap_arg_struct32* arg) {
    ...
    if (TestControl.verbos) {
        klog("%d: mmap: fd %d, addr %x, offset %x, len %x at addr %x\n", CURRENT_TASK()->psid, arg->fd, arg->addr,
             arg->offset, arg->len, vir);
    }
...
static int sys_fstat(int fd, struct stat* s) {
    ...
    if (TestControl.verbos) {
        klog("%d: sys_fstat(%d, %x) size %d isdir %d blksize %d\n", CURRENT_TASK()->psid, fd, s, s->st_size,
             S_ISDIR(s->st_mode), s->st_blksize);
    }
    return ret;
}
...
static int sys_fstat64(int fd, struct stat64* s) {
    ...
    if (TestControl.verbos) {
        klog("%d: fstat64(%d, %x) size %d blocks %d blksize %d\n", CURRENT_TASK()->psid, fd, s, s32.st_size,
             s32.st_blocks, s32.st_blksize);
    }
...
static int sys_llseek(int fd, unsigned offset_high, unsigned offset_low, uint64_t* result, unsigned whence) {
    int ret;
    ret = fs_llseek(fd, offset_high, offset_low, result, whence);
    if (TestControl.verbos) {
        klog("%d: llseek(%d, %x, %x, %x, %d) = %d, current %d\n", CURRENT_TASK()->psid, fd, offset_high, offset_low,
             result, whence, ret, CURRENT_TASK()->fds[fd].file_off);
    }
    return ret;
}
...
static int sys_select(int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, const struct timespec* timeout) {
    ...
    if (TestControl.verbos) {
        klog("%d: select(%d, %x, %x, %x, %x) = %d\n", CURRENT_TASK()->psid, nfds, readfds, writefds, exceptfds, timeout,
             ret);
    }
    return ret;
}
...
static int sys_newselect(int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, const struct timespec* timeout,
    ...
    if (TestControl.verbos) {
        klog("%d: pselect(%d, %x, %x, %x, %x, %x) = %d\n", CURRENT_TASK()->psid, nfds, readfds, writefds, exceptfds,
             timeout, sigmask, ret);
    }
}
