...
#define PIPE_REF_INCREASE(b) __sync_add_and_fetch(&(b->ref_count), 1)

#define PIPE_REF_DECREASE(b) __sync_add_and_fetch(&(b->ref_count), -1)

#define PIPE_LEN_ADD(b, len) __sync_add_and_fetch(&(b->length), len)

#define PIPE_LEN(b) __sync_add_and_fetch(&(b->length), 0)

#define PIPE_WRITER_INCREASE(b) __sync_add_and_fetch(&(b->writer_count), 1)

#define PIPE_WRITER_DECREASE(b) __sync_add_and_fetch(&(b->writer_count), -1)

#define PIPE_WRITERS(b) __sync_add_and_fetch(&(b->writer_count), 0)

#define PIPE_READER_INCREASE(b) __sync_add_and_fetch(&(b->reader_count), 1)

#define PIPE_READER_DECREASE(b) __sync_add_and_fetch(&(b->reader_count), -1)

#define PIPE_READERS(b) __sync_add_and_fetch(&(b->reader_count), 0)
