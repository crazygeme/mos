...
static void ansi_control_add(char c)
{
	...
	case 'm':
		...
		} else if (arg >= 30 && arg <= 37) { // ����ǰ��ɫ
		} else if (arg >= 40 && arg <= 47) { // ���ñ���ɫ
		}
		break;
	case 'A': // ��������n��
		arg = atoi(str_arg);
...
void *malloc(unsigned size)
{
	...
	if (free_list_head->next == NULL) {
		for (index = free_list_index + 1; index <= 512; index++) {
			...
			klib_add_to_free_list(left_index,
...
static int klib_free_list_merge(int node_index, kblock *buf, int target)
{
	...
	if (pre != head) {
		...
		if ((pre_addr + 8 * (target + 1) == buf_addr) &&
		    (merged_index <= 511) &&
	...
	if (!merged && node != NULL) {
		merged_index = node_index + target + 1;
		if ((buf_addr + 8 * (node_index + 1) == node_addr) &&
		    (merged_index <= 511) &&
...
uint64_t __udivmoddi4(uint64_t num, uint64_t den, uint64_t *rem_p)
{
	...
	if (den == 0) {
		return 1 /
		       ((unsigned)den); /* Intentional divide by zero, without
