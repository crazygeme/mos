...
static void vm_print(vm_struct_t vm)
{
	...
	while (pair) {
		...
		printf("%x - %x, off %x\t, node %x\n", region->begin,
		       region->end, region->offset, region->node);
...
static void test_case_1()
{
	...
	vm = vm_create();
	vm_add_map(vm, USER_ZONE_BEGIN + 3 * PAGE_SIZE,
		   USER_ZONE_BEGIN + 5 * PAGE_SIZE, 0, 0);
...
static void test_case_3()
{
	...
	vm_add_map(vm, USER_ZONE_BEGIN + 3 * PAGE_SIZE,
		   USER_ZONE_BEGIN + 6 * PAGE_SIZE, 0, 0);
	vm_add_map(vm, USER_ZONE_BEGIN + 0 * PAGE_SIZE,
		   USER_ZONE_BEGIN + 5 * PAGE_SIZE, 0, 0);
...
static void test_case_5()
{
	...
	vm_add_map(vm, USER_ZONE_BEGIN + 2 * PAGE_SIZE,
		   USER_ZONE_BEGIN + 5 * PAGE_SIZE, 0, 0);
	vm_add_map(vm, USER_ZONE_BEGIN + 0 * PAGE_SIZE,
		   USER_ZONE_BEGIN + 6 * PAGE_SIZE, 0, 0);
...
static void test_case_6()
{
	...
	vm_add_map(vm, USER_ZONE_BEGIN, USER_ZONE_BEGIN + 3 * PAGE_SIZE, 0, 0);
	vm_add_map(vm, USER_ZONE_BEGIN + PAGE_SIZE,
		   USER_ZONE_BEGIN + 2 * PAGE_SIZE, 0, 0);
...
static void test_find_2()
{
	...
	vm = vm_create();
	vm_add_map(vm, USER_ZONE_BEGIN, USER_ZONE_BEGIN + 3 * PAGE_SIZE, 0, 0);
	vm_add_map(vm, USER_ZONE_BEGIN + 4 * PAGE_SIZE,
		   USER_ZONE_BEGIN + 6 * PAGE_SIZE, 0, 0);
