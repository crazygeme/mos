...
KTEST(cyclebuf, putc_overflow_drops_oldest)
{
	...
	unsigned char first = cyb_getc(b);
	EXPECT_EQ(first, 1);
...
KTEST(cyclebuf, getbuf_eof_after_writer_close)
{
	...
	cyb_writer_close(b);
	int n = cyb_getbuf(b, dst, sizeof(dst));
	EXPECT_EQ(n, 0); /* 0 = EOF */
